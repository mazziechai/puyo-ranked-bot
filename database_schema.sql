...
-- Table: players
CREATE TABLE players (id BIGINT PRIMARY KEY ON CONFLICT ROLLBACK NOT NULL, registration_date DATETIME DEFAULT (datetime('now')) NOT NULL, platforms STRING NOT NULL, display_name STRING UNIQUE ON CONFLICT ROLLBACK, rating_mu DOUBLE DEFAULT (1500) NOT NULL, rating_phi DOUBLE DEFAULT (350) NOT NULL, rating_sigma DOUBLE NOT NULL DEFAULT (0.06), username_pc STRING UNIQUE ON CONFLICT ROLLBACK, username_switch STRING UNIQUE ON CONFLICT ROLLBACK, username_ps4 STRING UNIQUE ON CONFLICT ROLLBACK);
